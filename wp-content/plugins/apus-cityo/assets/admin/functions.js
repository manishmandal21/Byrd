...
jQuery(document).ready(function($){
    ...
    // custom fields
    $(document).on('click', '.apuscityo-custom-field-add-available-field', function () {
        "use strict";
        ...
        var ajax_url = apuscityo_customfield_common_vars.ajax_url,
                $this = $(this),
        ...
        var action = 'apuscityo_custom_field_available_html';
    ...
    $(document).on('click', '.btn-add-field', function () {
        ...
        var ajax_url = apuscityo_customfield_common_vars.ajax_url,
                $this = $(this),
                randid = $this.data('randid'),
                select_element = $('.apuscityo-field-types'),
        ...
        var old_text =  $this.html();
        $this.html('<i class="dashicons dashicons-update fa-spin"></i>');
        action = 'apuscityo_custom_field_html';
        var dataString = 'action=' + action + '&fieldtype=' + fieldtype + '&global_custom_field_counter=' + global_custom_field_counter;
    ...
    $(document).on('click', '.custom-fields-remove', function () {
        ...
        $('.custom-field-class-' + randid).slideUp("normal").promise().done(function () {
            $(this).remove();
            $('.apuscityo-form-field-list ul li.'+ fieldtype).removeClass('disabled');
    ...
    $(document).on('keyup change', '.apuscityo-custom-field-label', function(e){
        var parent = $(this).parents('.apuscityo-custom-field-container');
        parent.find('.field-intro a b').text( '(' + $(this).val() + ')' );
    });

    $(document).on('keyup change', '.apuscityo-custom-field-key', function(e){
        if (!apuscityo_isArrowKey(e)) {
            var new_val = apuscityo_str2url($(this).val().replace(/^[0-9]+\./, ''), 'UTF-8');
            $(this).val(new_val);
            var check = apuscityo_check_key_available(new_val);
            if ( check ) {
    ...
    function apuscityo_isArrowKey(k_ev)
    {
    ...
    function apuscityo_str2url(str, encoding, ucfirst)
    {
        str = str.toUpperCase();
        str = str.toLowerCase();
    ...
    function apuscityo_check_key_available($key) {
        var $i = 0;
        $('.apuscityo-custom-field-key').each(function(){
            if ( $key === $(this).val() ) {
                $i = $i + 1;
            }
        });
