...
class Cityo_WP_Private_Message_Message {
	
	public static function init() {
		add_action( 'wp_ajax_cityo_wp_private_message_send_message',  array(__CLASS__,'process_send_message') );
		add_action( 'wp_ajax_nopriv_cityo_wp_private_message_send_message',  array(__CLASS__,'process_send_message') );

		add_filter('cityo_get_default_blocks_sidebar_content', array(__CLASS__, 'blocks_content'), 100 );
		add_filter('cityo_get_default_blocks_sidebar_content', array(__CLASS__, 'blocks_content'), 100 );
		add_filter('cityo_get_default_blocks_sidebar_content', array(__CLASS__, 'blocks_content'), 100 );
		add_filter('cityo_get_default_blocks_sidebar_content', array(__CLASS__, 'blocks_content'), 100 );
	}
	...
	public static function process_send_message() {
		...
		if ( !isset( $_POST['wp-private-message-send-message-nonce'] ) || ! wp_verify_nonce( $_POST['wp-private-message-send-message-nonce'], 'wp-private-message-send-message' )  ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Sorry, your nonce did not verify.', 'cityo') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		if ( !is_user_logged_in() ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Please login to send a message.', 'cityo') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		...
		if ( empty($user->ID) ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Recipient did not exists.', 'cityo') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		...
		if ( empty($listing) || $listing->post_type !== 'job_listing' ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Listing did not exists.', 'cityo') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		...
		if ( $parent ) {
	        ...
	        if ( $message_id ) {
		        ...
		        $return = array( 'status' => true, 'msg' => esc_html__('Sent message successful.', 'cityo') );
			   	echo wp_json_encode($return);
			   	exit;
		    } else {
				$return = array( 'status' => false, 'msg' => esc_html__('Send message error.', 'cityo') );
			   	echo wp_json_encode($return);
			   	exit;
			}
		}
	...
	public static function process_reply_message($parent, $post_parent) {
        ...
        if ( $reply_id ) {
			...
			$return = array( 'status' => false, 'msg' => esc_html__('Reply message error.', 'cityo') );
		   	echo wp_json_encode($return);
		   	exit;
		}
	}
	...
	public static function blocks_content($contents) {
		$contents['private-message'] = esc_html__( 'Private Message', 'cityo' );
		return $contents;
	}
}
Cityo_WP_Private_Message_Message::init();