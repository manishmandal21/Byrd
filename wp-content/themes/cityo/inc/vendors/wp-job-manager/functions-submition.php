...
class Cityo_Submittion{
	...
	public static function init() {
		...
		if ( function_exists('apuscityo_addmetaboxes') ) {
			apuscityo_addmetaboxes( array( __CLASS__, 'metaboxes' ) );
		}

		add_filter('apuscityo-types-add_custom_fields', array( __CLASS__, 'admin_fields' ), 10 );
		// admin fields
		add_action( 'job_manager_input_cityo-location', array( __CLASS__, 'input_location_fields' ), 10, 2 );
		...
		add_action( 'job_manager_input_cityo-regions', array( __CLASS__, 'input_regions_fields' ), 10, 2 );
		...
		// region field
		add_action( 'wp_ajax_cityo_process_change_region', array( __CLASS__, 'process_change_region' ) );
		add_action( 'wp_ajax_nopriv_cityo_process_change_region', array( __CLASS__, 'process_change_region' ) );
	...
	public static function change_post_type_label($args) {
		$singular = esc_html__( 'Listing', 'cityo' );
		$plural   = esc_html__( 'Listings', 'cityo' );

		$args['labels']      = array(
			'name'               => $plural,
			'singular_name'      => $singular,
			'menu_name'          => $plural,
			'all_items'          => sprintf( esc_html__( 'All %s', 'cityo' ), $plural ),
			'add_new'            => esc_html__( 'Add New', 'cityo' ),
			'add_new_item'       => sprintf( esc_html__( 'Add %s', 'cityo' ), $singular ),
			'edit'               => esc_html__( 'Edit', 'cityo' ),
			'edit_item'          => sprintf( esc_html__( 'Edit %s', 'cityo' ), $singular ),
			'new_item'           => sprintf( esc_html__( 'New %s', 'cityo' ), $singular ),
			'view'               => sprintf( esc_html__( 'View %s', 'cityo' ), $singular ),
			'view_item'          => sprintf( esc_html__( 'View %s', 'cityo' ), $singular ),
			'search_items'       => sprintf( esc_html__( 'Search %s', 'cityo' ), $plural ),
			'not_found'          => sprintf( esc_html__( 'No %s found', 'cityo' ), $plural ),
			'not_found_in_trash' => sprintf( esc_html__( 'No %s found in trash', 'cityo' ), $plural ),
			'parent'             => sprintf( esc_html__( 'Parent %s', 'cityo' ), $singular )
		);
		$args['description'] = sprintf( esc_html__( 'This is where you can create and manage %s.', 'cityo' ), $plural );
	...
	public static function add_custom_fields($fields) {
		if ( cityo_get_config('claim_enable') ) {
			$fields['_claimed'] = array(
				'label'       => esc_html__( 'Claimed:', 'cityo' ),
				'type'        => 'checkbox',
				'description' => esc_html__( 'The owner has been verified.', 'cityo' ),
				'priority'    => 2.3,
				'required' => false,
			);
		}
		
		$types = cityo_get_listing_layout_style();
		$opts = array('' => esc_html__('Global Settings', 'cityo'));
		foreach ($types as $key => $value) {
			$opts[$key] = $value['title'];
		}
		$fields['_layout_type'] = array(
			'label'       => esc_html__( 'Layout Type', 'cityo' ),
			'type'        => 'select',
			'description' => esc_html__( 'Choose layout for listing', 'cityo' ),
			'priority'    => 100,
			'required' => false,
			'options' => $opts
		);
	...
	public static function metaboxes( $post_type ) {
        ...
        if ( in_array( $post_type, $post_types ) && function_exists('apuscityo_addmetabox') ) {

            apuscityo_addmetabox(
            	'job_listing_images',
                esc_html__( 'Listing Images', 'cityo' ),
            ...
            if ( class_exists('ApusCityo_Custom_Fields') ) {
	            $fields = ApusCityo_Custom_Fields::add_custom_fields(array());
	            if ( !empty($fields) ) {
	            	foreach ($fields as $key => $field) {
	            		if ( $field['type'] == 'repeater' ) {
	            			$label = !empty($field['label']) ? $field['label'] : esc_html__('Repeater', 'cityo');
	            			apuscityo_addmetabox(
	            				'job_listing_'.$key,
				                $label,
	            		...
	            		} elseif ( $field['type'] == 'cityo-menu-prices' ) {
	            			$label = !empty($field['label']) ? $field['label'] : esc_html__('Menu Prices', 'cityo');
	            			apuscityo_addmetabox(
	            				'job_listing_'.$key,
				                $label,
	            		...
	            		} elseif ( $field['type'] == 'cityo-hours' ) {
	            			$label = !empty($field['label']) ? $field['label'] : esc_html__('Hours Operation', 'cityo');
	            			apuscityo_addmetabox(
	            				'job_listing_'.$key,
				                $label,
	...
	public static function render_meta_box_repeater( $post, $args ) {
		...
		cityo_repeater_rows_html( $field['label'], $field['fields'], get_post_meta( $post->ID, $key, true ) );
	}
	...
	public static function render_meta_box_settings( $post ) {
		...
		if ( !empty($post) ) {
			...
			$fields = get_option('custom_'.$apus_cityo_listing_type.'_fields_data', true);
			$has_job_logo = self::check_type_has_field($fields, 'job_logo');
			if ( $has_job_logo ) {
				$job_logo = get_post_meta($post->ID, '_job_logo', true);

    	?>
		    	<div class="cityo-listing-side-meta">
		    		<h3><?php esc_html_e('Listing Logo', 'cityo'); ?></h3>
		    		...
		    		<button id="upload-image-logo" type="button" class="button"><?php esc_html_e('Add logo image', 'cityo'); ?></button>
    	...
    	?>		
		    	<div class="cityo-listing-side-meta">
		    		<h3><?php esc_html_e('Gallery Images', 'cityo'); ?></h3>
		    		...
		    		<button id="upload-image-gallery" type="button" class="button"><?php esc_html_e('Add gallery images', 'cityo'); ?></button>
		    	</div>
	    	<?php }
	    	?>
	    <?php } ?>
	...
	public static function input_hours_fields($key, $field) {
		...
		<div class="form-field full-form-field">
			...
			get_job_manager_template( 'form-fields/cityo-hours-field.php', array('key' => $key, 'field' => $field) );
			?>
		</div>
	...
	public static function input_location_fields($key, $field) {
		...
		<div class="form-field location-form-field full-form-field">
			...
			get_job_manager_template( 'form-fields/cityo-location-field.php', array('key' => $key, 'field' => $field) );
			?>
		</div>
	...
	public static function input_menu_prices_fields($key, $field) {
		...
		<div class="form-field full-form-field">
			...
			get_job_manager_template( 'form-fields/cityo-menu-prices-field.php', array('key' => $key, 'field' => $field) );
			?>
		</div>
	...
	public static function input_regions_fields($key, $field) {
		...
		<div class="form-field">
			...
			get_job_manager_template( 'form-fields/cityo-regions-field.php', array('key' => $key, 'field' => $field) );
			?>
		</div>
	...
	public static function process_change_region() {
		...
		$region_text = cityo_get_config('submit_listing_region_'.$next.'_field_label');
		$placeholder = !empty($_POST['placeholder']) ? $_POST['placeholder'] : sprintf(esc_html__('Please select %s', 'cityo'), $region_text);
	...
	public static function listing_submit( $id, $values ) {
		if ( isset($values[ 'job' ][ 'job_cover_image' ]) && !empty($values[ 'job' ][ 'job_cover_image' ]) ) {
			...
			if ( is_numeric($job_cover_image) ) {
				...
				$attachment_id = cityo_get_attachment_id_from_url( $job_cover_image );
			}
		...
		if ( isset($values[ 'job' ][ 'job_logo' ]) && !empty($values[ 'job' ][ 'job_logo' ]) ) {
			...
			if ( is_numeric($job_logo) ) {
				...
				$attachment_id = cityo_get_attachment_id_from_url( $job_logo );
			}
		...
		if ( isset($values[ 'job' ][ 'job_gallery_images' ]) && !empty($values[ 'job' ][ 'job_gallery_images' ]) ) {
			...
			if ( is_numeric( $job_gallery_images ) ) {
				$attach_id = cityo_get_attachment_id_from_url( $job_gallery_images );
				...
				foreach ( $job_gallery_images as $key => $url ) {
					if ( is_numeric( $url ) ) {
						...
						$attach_id = cityo_get_attachment_id_from_url( $url );
						if ( ! empty( $attach_id ) && is_numeric( $attach_id ) ) {
							$gallery_images[] = $attach_id;
						}
					}
	...
	public static function listing_update( $id, $values ) {
		...
		if ( class_exists('ApusCityo_Custom_Fields') ) {
	        ...
	        if ( !empty($fields) ) {
	        	foreach ($fields as $key => $field) {
	        		if ( $field['type'] == 'repeater' ) {
	        			cityo_save_repeater_row( $id, $key, $field['fields'] );
	        		}
	        	}
	        }
        }
	...
	public static function job_manager_dropdown_categories( $args = '' ) {
		$defaults = array(
			...
			'placeholder'     => __( 'Choose a category&hellip;', 'cityo' ),
			'no_results_text' => __( 'No results match', 'cityo' ),
			'multiple_text'   => __( 'Select Some Options', 'cityo' ),
			'meta_query'   => ''
		);
