...
class Cityo_Type_Real_Estate{

	public static function init() {
		
		// required/available form
		add_filter( 'apuscityo-real-estate-type-required-fields', array( __CLASS__, 'required_fields' ), 100 );
		add_filter( 'apuscityo-real-estate-type-available-fields', array( __CLASS__, 'available_fields' ), 100 );
		...
		add_filter( 'apuscityo_list_simple_type', array( __CLASS__, 'field_virtual_tour' ) );
	}
	...
	public static function available_fields($dfields) {
		...
		if ( get_option( 'job_manager_enable_types' ) ) {
			$fields['job_type'] = array(
				...
				'placeholder' => esc_html__( 'Add Type', 'cityo' ),
				'label' => esc_html__( 'Listing Type', 'cityo' ),
			);
		}
		...
		$fields['job_virtual_tour'] = array(
			'label'       => esc_html__( 'Virtual Tour (360) url', 'cityo' ),
			'type'        => 'text',
			'required'    => false,
			'placeholder' => esc_html__( 'add a virtual tour url', 'cityo' ),
		);
		// socials
...
For Buy',
		...
		$fields['job_rooms'] = array(
			'label'       => esc_html__( 'Rooms', 'cityo' ),
			'type'        => 'text',
			'required'    => false,
			'placeholder' => esc_html__( 'e.g. 3', 'cityo' ),
		);

		$fields['job_beds'] = array(
			'label'       => esc_html__( 'Beds', 'cityo' ),
			'type'        => 'text',
			'required'    => false,
			'placeholder' => esc_html__( 'e.g. 3', 'cityo' ),
		);

		$fields['job_baths'] = array(
			'label'       => esc_html__( 'Baths', 'cityo' ),
			'type'        => 'text',
			'required'    => false,
			'placeholder' => esc_html__( 'e.g. 3', 'cityo' ),
		);

		$fields['job_garages'] = array(
			'label'       => esc_html__( 'Garages', 'cityo' ),
			'type'        => 'text',
			'required'    => false,
			'placeholder' => esc_html__( 'e.g. 2', 'cityo' ),
		);

		$fields['job_home_area'] = array(
			'label'       => esc_html__( 'Home area', 'cityo' ),
			'type'        => 'text',
			'required'    => false,
			'placeholder' => sprintf(esc_html__( 'e.g. 200 (%s)', 'cityo' ), cityo_get_config('listing_area_unit', 'sqft') ),
		);

		$fields['job_lot_demensions'] = array(
			'label'       => esc_html__( 'Lot dimensions', 'cityo' ),
			'type'        => 'text',
			'required'    => false,
			'placeholder' => sprintf(esc_html__( 'e.g. 20x30, 20x30x40, 20x30x40x50 (%s)', 'cityo' ), cityo_get_config('listing_distance_unit', 'ft') ),
		);

		$fields['job_lot_area'] = array(
			'label'       => esc_html__( 'Lot area', 'cityo' ),
			'type'        => 'text',
			'required'    => false,
			'placeholder' => sprintf(esc_html__( 'e.g. 300 (%s)', 'cityo' ), cityo_get_config('listing_area_unit', 'sqft') ),
		);

		$fields['job_year_built'] = array(
			'label'       => esc_html__( 'Year Build', 'cityo' ),
			'type'        => 'text',
			'required'    => false,
			'placeholder' => esc_html__( 'e.g. 2010', 'cityo' ),
		);

		$fields['job_floor_plans'] = array(
			'label' => esc_html__( 'Floor Plans', 'cityo' ),
			'priority' => 2.9,
			'required' => false,
			'type' => 'file',
			'ajax' => true,
	...
	public static function blocks_content() {
	    return apply_filters( 'cityo_listing_single_place_content', array(
	        ...
	        'virtual-tour' => esc_html__( 'Virtual Tour', 'cityo' ),
	        'floor-plans' => esc_html__( 'Floor Plans', 'cityo' ),
	...
	public static function blocks_sidebar_content() {
	    return apply_filters( 'cityo_listing_single_place_sidebar', array(
	        'detail-real-estate' => esc_html__( 'Detail', 'cityo' ),
	...
	public static function custom_fields_display_hooks($hooks) {
		...
		$hooks['cityo-single-listing-real-estate-detail'] = esc_html__('Single Listing - Detail Property', 'cityo');
	...
	public static function display_custom_field_data($html, $custom_field, $post, $field_label, $output_value, $current_hook) {
		if ( $current_hook === 'cityo-single-listing-real-estate-detail' ) {
			ob_start();
			?>
	...
	public static function elementor_widgets() {
		get_template_part( 'inc/vendors/wp-job-manager/real-estate/widgets/listings' );
		get_template_part( 'inc/vendors/wp-job-manager/real-estate/widgets/search_form' );
	}
	...
	public static function get_job_listings_query_args($query_args, $args, $datas) {
		...
		if ( !empty( $datas['search_rooms'] ) ) {
		    $meta_query[] = array(
			    'key'       => '_job_rooms',
			    'value'     => $datas['search_rooms'],
			    'compare'   => '>=',
			    'type'      => 'NUMERIC',
		    );
	    }
	    if ( !empty( $datas['search_beds'] ) ) {
		    $meta_query[] = array(
			    'key'       => '_job_beds',
			    'value'     => $datas['search_beds'],
			    'compare'   => '>=',
			    'type'      => 'NUMERIC',
		    );
	    }
		if ( !empty( $datas['search_baths'] ) ) {
		    $meta_query[] = array(
			    'key'       => '_job_baths',
			    'value'     => $datas['search_baths'],
			    'compare'   => '>=',
			    'type'      => 'NUMERIC',
		    );
	    }
	    if ( !empty( $datas['search_garages'] ) ) {
		    $meta_query[] = array(
			    'key'       => '_job_garages',
			    'value'     => $datas['search_garages'],
			    'compare'   => '>=',
			    'type'      => 'NUMERIC',
		    );
	    }
	    if (isset($datas['search_home_area']) && $datas['search_home_area']) {
			$home_areas = explode('-', $datas['search_home_area']);
			$home_areas = array_map('trim', $home_areas);

			if ( count($home_areas) == 2 ) {
				$meta_query[] = array(
		           'key' => '_job_home_area',
		           'value' => $home_areas,
		           'type'    => 'NUMERIC',
		...
		if (isset($datas['search_lot_area']) && $datas['search_lot_area']) {
			$lot_areas = explode('-', $datas['search_lot_area']);
			$lot_areas = array_map( 'trim', $lot_areas);
			if ( count($lot_areas) == 2 ) {
				$meta_query[] = array(
		           'key' => '_job_lot_area',
		           'value' => $lot_areas,
		           'type'    => 'NUMERIC',
	...
	public static function field_virtual_tour($fields) {
		$fields[] = 'job_virtual_tour';
		return $fields;
	}
	
}

Cityo_Type_Real_Estate::init();