...
function cityo_wrap_the_listings( $html ) {
	$output = '';
...
add_filter( 'job_manager_job_listings_output', 'cityo_wrap_the_listings', 10, 1 );
...
function cityo_listing_template( $template ) {
	global $wp_query;
...
add_filter( 'template_include', 'cityo_listing_template' );



function cityo_get_job_listings_query_args($query_args, $args) {
	global $wpdb, $wp_query, $cityo_distances;
	$cityo_distances = array();
	...
	if (isset($_REQUEST['form_data']) || isset($_REQUEST['$form_data'])) {
		...
		if ( isset( $datas['filter_order'] ) ) {
			if ( 'default' === $datas['filter_order'] ) { // Default show featured.
				...
				$query_args['cityo_proximity_filter'] = false;
				$query_args = cityo_sort_listings_query( $query_args, $datas['filter_order'] );
			}
		}
		...
		$query_args = apply_filters('cityo_get_job_listings_query_args', $query_args, $args, $datas);
		...
		$earth_distance = cityo_get_config('listing_filter_distance_unit', 'km') == 'miles' ? 3959 : 6371;
		...
		$query_args = cityo_remove_location_meta_query( $query_args );
	}
	
	return $query_args;
}
add_filter( 'get_job_listings_query_args', 'cityo_get_job_listings_query_args', 10, 2 );



function cityo_sort_listings_query( $query_args, $sort_option ) {
...
function cityo_job_listings_cache_results( $return ) {
	if (isset($_REQUEST['form_data'])) {
		$form_data = urldecode($_REQUEST['form_data']);
		parse_str($form_data, $datas);
...
add_filter('get_job_listings_cache_results', 'cityo_job_listings_cache_results', 100);

function cityo_remove_location_meta_query( $query_args ) {
	$found = false;
...
function cityo_job_manager_get_listings_args( $args ) {
	if ( ! isset( $_REQUEST[ 'form_data' ] ) ) {
		return $args;
	}
...
add_filter( 'job_manager_get_listings_args', 'cityo_job_manager_get_listings_args' );

function cityo_get_listings_custom_filter_text($text) {
	$after_text = array();
	if (isset($_REQUEST['form_data'])) {
		$form_data = urldecode($_REQUEST['form_data']);
		parse_str($form_data, $datas);
		...
		if (isset($datas['search_categories']) && $datas['search_categories']) {
			...
			if ( !empty($categories) ) {
				$after_text[] = sprintf( _n('<span class="text-strong">Category</span>: %s', '<span class="text-strong">Categories</span>: %s', count($categories), 'cityo'), implode(', ', $categories) );
			}
		}
		if (isset($datas['job_region_select']) && $datas['job_region_select']) {
			if ( is_array($datas['job_region_select']) ) {
			...
			if (!empty($regions)) {
				$after_text[] = sprintf(_n('<span class="text-strong">Region</span>: %s', '<span class="text-strong">Regions</span>: %s', count($regions), 'cityo'), implode(', ', $regions) );
			}
		}
		if (isset($datas['job_type_select']) && $datas['job_type_select']) {
			$term = get_term_by( 'id', $datas['job_type_select'], 'job_listing_type' );
			if (!empty($term)) {
				$after_text[] = '<span class="text-strong">'.esc_html__('Type', 'cityo').'</span>: <span class="highlight">'.$term->name.'</span>';
			}
		}
		if (isset($datas['filter_job_amenity']) && $datas['filter_job_amenity']) {
			$amenities = array();
			...
			$after_text[] = sprintf(_n('<span class="text-strong">Amenity</span>: %s', '<span class="text-strong">Amenities</span>: %s', count($amenities), 'cityo'), implode(', ', $amenities) );
		}
		if (!empty($datas['filter_job_tag'])) {
			...
			$after_text[] = sprintf(_n('<span class="text-strong">Tag</span>: %s', '<span class="text-strong">Tags</span>: %s', count($tags), 'cityo'), implode(', ', $tags) );
		}
		
		$location = isset( $datas[ 'search_location' ] ) ? esc_attr( $datas[ 'search_location' ] ) : false;

		if ( $location ) {
			$after_text[] = '<span class="text-strong">'.esc_html__('Location', 'cityo').'</span>: <span class="highlight">'.$location.'</span>';
		}

		if (isset($datas['filter_price_range']) && $datas['filter_price_range']) {
			$prices_default = cityo_job_manager_price_range_icons();
			$prices = array();
			...
			$after_text[] = '<span class="text-strong">'.esc_html__('Price Range', 'cityo').'</span>: '.implode(', ', $prices);
		...
		if ( !empty($datas['filter_event_date_from']) && !empty($datas['filter_event_date_to']) ) {
			...
			$after_text[] = '<span class="text-strong">'.esc_html__('Date', 'cityo').'</span>: '.$date_html;

		}

		if ( !empty($datas['search_contract']) ) {
			$after_text[] = '<span class="text-strong">'.esc_html__('Contract', 'cityo').'</span>: '.$datas['search_contract'];
		}
		if ( !empty($datas['filter_price_from']) && !empty($datas['filter_price_to']) ) {
			...
			$after_text[] = '<span class="text-strong">'.esc_html__('Price', 'cityo').'</span>: '.$price_html;
		}

	}

	if ( !empty($after_text) ) {
		return esc_html__('Showing results for ', 'cityo').' '.implode('; ', $after_text);
	} else {
		return '';
	}
}
add_filter( 'job_manager_get_listings_custom_filter_text', 'cityo_get_listings_custom_filter_text' );

function cityo_listings_custom_filter() {
	if (isset($_REQUEST['form_data'])) {
		$form_data = urldecode($_REQUEST['form_data']);
		parse_str($form_data, $datas);
...
add_filter( 'job_manager_get_listings_custom_filter', 'cityo_listings_custom_filter' );

function cityo_listings_remove_rss_link($links) {
	if ( isset($links['rss_link']) ) {
		unset($links['rss_link']);
	}
	return $links;
}
add_filter( 'job_manager_job_filters_showing_jobs_links', 'cityo_listings_remove_rss_link' );


function cityo_listings_json_results( $result, $jobs ) {
	...
	if ( cityo_get_config('listing_show_loadmore') ) {
		$result[ 'str_found' ] = esc_html__(' (Showing ', 'cityo'). ($total_found > 0 ? ' 1' : ' 0') . ' - ' . $total_found . ')';
	} else {
		$result[ 'str_found' ] = esc_html__(' (Showing ', 'cityo'). ($total_found > 0 ? intval($jobs->query_vars[ 'offset' ]) + 1 : ' 0') . ' - '. $total_found .')';
	}

	return $result;
}
add_filter( 'job_manager_get_listings_result', 'cityo_listings_json_results', 10, 2 );

function cityo_job_manager_output_jobs_defaults($args) {
	$args['regions'] = '';
	return $args;
}
add_filter('job_manager_output_jobs_defaults', 'cityo_job_manager_output_jobs_defaults');
