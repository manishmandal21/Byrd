...
if ( ! function_exists( 'cityo_get_post_image_src' ) ) {
	function cityo_get_post_image_src( $post_id = null, $size = 'thumbnail' ) {
		if ( empty( $post_id ) ) {
			$post_id = get_the_ID();
		}
		...
		if ( !empty($attach_id) ) {
			$gallery_ids = cityo_get_listing_gallery( $post_id );
...
if ( ! function_exists( 'cityo_get_listing_gallery' ) ) {
	function cityo_get_listing_gallery( $post_id ) {
		$ids = array();
		if ( has_post_thumbnail( $post_id ) ) {
			$attach_id = esc_sql( get_post_thumbnail_id( $post_id ) );
			$ids[] = $attach_id;
...
function cityo_sort_array_by_priority( $a, $b ) {
...
function cityo_get_current_time() {
	global $wp_locale;
...
function cityo_get_current_time_status($listing_id) {
	$current = cityo_get_current_time();
	$current_day = strtolower($current['day']);
	$current_time = $current['time'];
	$hours = get_post_meta( $listing_id, '_job_hours', true );
	
	if ( !empty($hours['day']) ) {
		$days = cityo_get_day_hours($hours['day']);
...
function cityo_get_day_hours($hours) {
	global $wp_locale;
	if (empty($hours) || !is_array($hours)) {
		return;
	}
    $numericdays = cityo_get_days_of_week();
    $days = array();
        	...
        	if ( !empty($hours[ $i ][ 'from' ]) && !empty($hours[ $i ][ 'to' ]) ) {
        		...
        		foreach ($hours[ $i ][ 'from' ] as $key => $value) {
        			...
        			if ( $start && $end ) {
        				...
        				$end = strtotime($end);
	        			$t_day[] = array( date_i18n(get_option('time_format'), $start), date_i18n(get_option('time_format'), $end) );
	        		}
        		}
        		$days[ $day ] = $t_day;
        	}
...
function cityo_check_menu_prices_empty($section) {
...
function cityo_check_event_schedule_empty($section) {
	$titles_value = !empty($section['title']) ? $section['title'] : array();
...
function cityo_price_format_number( $price ) {
	...
	$money_thousands_separator = cityo_get_config('listing_currency_thousands_separator', 0);
...
function cityo_get_field_options($field_type) {
	global $apus_cityo_listing_type;
	$fields = array();
	if ( !class_exists('ApusCityo_Custom_Fields') ) {
		return $fields;
	}
	$custom_all_fields = apuscityo_get_custom_fields_data($apus_cityo_listing_type);
	if (is_array($custom_all_fields) && sizeof($custom_all_fields) > 0) {
		
		$dtypes = apuscityo_get_all_field_types();
        $available_types = apuscityo_all_types_available_fields($apus_cityo_listing_type);
        $required_types = apuscityo_all_types_required_fields($apus_cityo_listing_type);
		...
		foreach ($custom_all_fields as $key => $custom_field) {
			...
			if ( $fieldkey === $field_type ) {
				if ( !empty($fieldkey) ) {
					...
					if ( $fieldtype ) {
						$fields = ApusCityo_Custom_Fields::render_field($field_data, $fieldkey, $fieldtype, 1);
					}
				}
				return $fields;
			}
...
function cityo_job_manager_dropdown_types( $args = '' ) {
	$defaults = array(
		'orderby'         => 'id',
		'order'           => 'ASC',
		'show_count'      => 0,
		...
		'placeholder'     => esc_html__( 'Choose a category&hellip;', 'cityo' ),
		'no_results_text' => esc_html__( 'No results match', 'cityo' ),
		'multiple_text'   => esc_html__( 'Select Some Options', 'cityo' ),
	);
	...
	if ( ! empty( $categories ) ) {
		...
		$walker = new Cityo_WP_Job_Manager_Category_Walker();
...
// amenity field
add_action( 'wp_ajax_cityo_process_change_category_amenity', 'cityo_process_change_category_amenity' );
add_action( 'wp_ajax_nopriv_cityo_process_change_category_amenity', 'cityo_process_change_category_amenity' );
function cityo_process_change_category_amenity() {
...
// amenity admin field
add_action( 'wp_ajax_cityo_process_change_category_amenity_admin', 'cityo_process_change_category_amenity_admin' );
add_action( 'wp_ajax_nopriv_cityo_process_change_category_amenity_admin', 'cityo_process_change_category_amenity_admin' );
function cityo_process_change_category_amenity_admin() {
...
add_action( 'wp_ajax_cityo_process_change_category_amenities', 'cityo_process_change_category_amenities' );
add_action( 'wp_ajax_nopriv_cityo_process_change_category_amenities', 'cityo_process_change_category_amenities' );
function cityo_process_change_category_amenities() {
	check_ajax_referer( 'cityo-ajax-nonce', 'security' );
	$category_parent = !empty($_POST['category_parent']) ? $_POST['category_parent'] : '';
	...
	?>
		...
		?>
		<div class="amenities-wrap">
			<div class="alert alert-warning"><?php esc_html_e('Please choose category to display filters', 'cityo'); ?></div>
		</div>
		<?php
	}
	die();
...
function cityo_all_countries() {
    $countries = array(
    	''	=> esc_html__('All Countries', 'cityo'),
        'af' => 'Afghanistan',
        'ax' => 'Islands',
        'al' => 'Albania',
        'dz' => 'Algeria',
    ...
    return apply_filters( 'cityo-all-countries', $countries );
}