...
class Cityo_Attachments {
    ...
    public static function displayUploadField() {
        global $post;
        $images_upload = cityo_get_config('listing_review_enable_upload_image', true);
        $html = '';
        ob_start();
        if ( $images_upload ) {
            ?>
            <div class="form-group group-upload">
                ...
                <button type="button" id="field_attachments_cover">
                    <span class="title-upload"><?php esc_html_e('Drop images to upload', 'cityo'); ?></span>
                    <span class="break"><?php esc_html_e('or', 'cityo'); ?></span>
                    <div class="upload-file-btn">
                        <i class="flaticon-upload"></i>
                        <span><?php esc_html_e('Gallery Images', 'cityo'); ?></span>
                    </div>
                </button>
    ...
    public static function notificationText($notify_message,  $comment_id)
    {
        ...
        if ( !empty($attachments) && is_array($attachments) ) {
        	$notify_message .= esc_html__('Attachment:', 'cityo');
        	foreach ($attachments as $attachmentId) {
            ...
            foreach ($files['name'] as $key => $value) {
                ...
                if ($files['size'][$key] > 0 && $files['error'][$key] == 0){
                    ...
                    if (!in_array($fileType, self::getMimeTypes()) || !in_array(strtolower($fileExtension), self::getAllowedFileExtensions()) || $files['size'][$key] > $max_size) { // file size from admin
                        $errors[] = sprintf(wp_kses(__('<strong>ERROR:</strong> File <strong>%1$s</strong> you upload must be valid file type <strong>(%2$s)</strong>, and under %3$s!','cityo'), $allowed_html_array), $files['name'][$key], implode( ', ', self::getMimeTypes()), ini_get('post_max_size'));
                    }

                    // error 4 is actually empty file mate
                } elseif($files['error'][$key] == 1) {
                    $errors[] = wp_kses(__('<strong>ERROR:</strong> The uploaded file exceeds the upload_max_filesize directive in php.ini.','cityo'), $allowed_html_array);
                } elseif($files['error'][$key] == 2) {
                    $errors[] = wp_kses(__('<strong>ERROR:</strong> The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form.','cityo'), $allowed_html_array);
                } elseif($files['error'][$key] == 3) {
                    $errors[] = wp_kses(__('<strong>ERROR:</strong> The uploaded file was only partially uploaded. Please try again later.','cityo'), $allowed_html_array);
                } elseif($files['error'][$key] == 6) {
                    $errors[] = wp_kses(__('<strong>ERROR:</strong> Missing a temporary folder.','cityo'), $allowed_html_array);
                } elseif($files['error'][$key] == 7) {
                    $errors[] = wp_kses(__('<strong>ERROR:</strong> Failed to write file to disk.','cityo'), $allowed_html_array);
                } elseif($files['error'][$key] == 7) {
                    $errors[] = wp_kses(__('<strong>ERROR:</strong> A PHP extension stopped the file upload.','cityo'), $allowed_html_array);
                }
