...
function cityo_repeater_rows_html( $group_name, $fields, $data ) {
	?>
	<div class="wc-job-manager-repeater-rows">
			...
			<?php
				if ( $data ) {
					foreach ( $data as $item ) {
						...
						echo '<a href="" class="delete-repeat-row">'.esc_html__('Delete this row', 'cityo').'</a>';
						echo '</div>';
					}
				}
			?>
		...
		<div class="submit">
			<input type="submit" class="button repeate_field_add_row" value="<?php printf(esc_attr__( 'Add %s', 'cityo' ), $group_name ); ?>" data-row="<?php
				ob_start();
				echo '<div class="repeater-field row">';
 ...
 * @since 1.11.3
 */
function cityo_save_repeater_row( $post_id, $meta_key, $fields ) {
	$items            = array();
...
// front end
function cityo_get_repeater_field( $field_prefix, $field ) {
	$fields = $field['fields'];
			...
			foreach ( $fields as $key => $field ) {
				...
				switch ( $field['type'] ) {
					...
					case 'file' :
						...
						$file = cityo_upload_file( $field_name, $field );
...
function cityo_upload_file( $field_key, $field ) {
...
function cityo_get_posted_repeater_field() {
	return 'cityo_get_repeater_field';
}
add_filter( 'job_manager_get_posted_repeater_field', 'cityo_get_posted_repeater_field' );