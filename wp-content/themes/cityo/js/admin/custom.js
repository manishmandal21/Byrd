	...
	jQuery(document).ready(function($) {
            ...
            if ( $(main_select).length > 0 ) {
                ...
                if ( val ) {
                    $.ajax({
                        ...
                        type:'POST',
                        dataType: 'html',
                        data:  'action=cityo_process_change_region&region_parent='+val+'&next='+next+'&name='+ name + '&security=' + cityo_opts.ajax_nonce
                    }).done(function(data) {
                    ...
                    if ( name == '_job_regions[]' || name == 'job_regions[]' ) {
                        html += '<label>' + cityo_opts.region_labels[next] +'</label>';
                    }
            ...
            if ( $('#_job_c_type').length > 0 ) {
                ...
                $.ajax({
                    ...
                    data:{
                        ...
                        'category_parent': val,
                        'security': cityo_opts.ajax_nonce,
        ...
        var change_category_amenities = function(val) {
            ...
            $.ajax({
                ...
                dataType: 'html',
                data:{
                    'action': 'cityo_process_change_category_amenity_admin',
                    'category_parent': val,
                    'security': cityo_opts.ajax_nonce,
                }
            }).done(function(data) {
        ...
        $("body").on("all_loading_icons", function(){
            cityo_get_awesome_icons();
            cityo_get_cityo_flaticon_icons();
        });

        function cityo_get_awesome_icons() {
        ...
        function cityo_get_cityo_flaticon_icons() {
            var files = document.querySelectorAll('*[id^="cityo-flaticon"]'), html = '', css;
            ...
            for( var i=0; i < files.length; i++ ){
                css = Array.prototype.map.call( files[i].sheet.cssRules, cityo_css_text ).join('\n');
            ...
            all_loaded_icons.cityo = icons_load_call;
        }
        function cityo_css_text(x) {
            return x.cssText;
        }
