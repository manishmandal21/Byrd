(function ($) {
	...
	var listingLocation = {
		...
		initializeMap: function() {
			var self = this;
			var maps = [];
			if ( $('.cityo-location-field').length > 0 ) {
				var mapInstance = $('.cityo-location-field');
				var geocoder_e = mapInstance.find( '.geocoder' );
				var mapCanvas = mapInstance.find( '.cityo-location-field-map' ).attr('id');
				...
				if ( cityo_listing_map_opts.geocoder_country ) {
                    geocoder = new L.Control.Geocoder.Nominatim({
                        geocodingQueryParams: {countrycodes: cityo_listing_map_opts.geocoder_country}
                    });
                } else {
                    geocoder = new L.Control.Geocoder.Nominatim();
                }
				map = L.map('cityo-location-field-map', {
	            ...
	            if ( cityo_listing_map_opts.map_service == 'mapbox' ) {
	                var tileLayer = L.tileLayer('https://api.mapbox.com/styles/v1/mapbox/'+cityo_listing_map_opts.mapbox_style+'/tiles/{z}/{x}/{y}?access_token='+ cityo_listing_map_opts.mapbox_token, {
	                    attribution: " &copy;  <a href='https://www.mapbox.com/about/maps/'>Mapbox</a> &copy;  <a href='http://www.openstreetmap.org/copyright'>OpenStreetMap</a> <strong><a href='https://www.mapbox.com/map-feedback/' target='_blank'>Improve this map</a></strong>",
	                    maxZoom: 18,
	                });
	            } else {
	                if ( cityo_listing_map_opts.custom_style != '' ) {
	                    try {
	                        var custom_style = $.parseJSON(cityo_listing_map_opts.custom_style);
	            ...
	            var mapPinHTML = "<div class='map-popup map-popup-empty'><div class='icon-wrapper'><div class='icon-cat'><i class='fa fa-map-pin'></i></div></div></div>";
	            ...
	            $(document).on('keydown', '.input-location-field',function search(e) {
