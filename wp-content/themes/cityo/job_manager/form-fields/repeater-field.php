				...
				<div class="repeater-field">
					...
					<?php foreach ( $field['fields'] as $subkey => $subfield ) : ?>
						<fieldset class="fieldset-<?php echo esc_attr( $subkey ); ?>">
							<label for="<?php echo esc_attr( $subkey ); ?>"><?php echo wp_kses_post($subfield['label']) . ( !empty($subfield['required']) && $subfield['required'] ? '' : ' <small>' . esc_html__( '(optional)', 'cityo' ) . '</small>' ); ?></label>
							<div class="field">
								<?php
									// Get name and value
					...
					<a href="#" class="delete-repeat-row"><?php esc_html_e( 'Remove', 'cityo' ); ?></a>
				</div>
			<?php endforeach; ?>
		<?php endif; ?>
	</div>
	<a href="#" class="repeate_field_add_row text-success" data-row="<?php
		...
		?>
			<div class="repeater-field">
				...
				<a href="#" class="delete-repeat-row text-danger"><?php esc_html_e( 'Remove', 'cityo' ); ?></a>
			</div>
		<?php
		echo esc_attr( ob_get_clean() );

	?>">+ <?php echo esc_html( ! empty( $field['add-row-text'] ) ? $field['add-row-text'] : esc_html__( 'Add Row', 'cityo' ) ); ?></a>
