...
<div class="hours-field <?php echo esc_attr( is_admin() ? 'is-admin' : ''); ?> group-fields">
	...
	<div class="hours-field-timezone hidden">
		...
		?>
		<label><?php esc_html_e( 'Timezone', 'cityo' ) ?></label>
		<select name="job_hours[timezone]" placeholder="<?php esc_attr_e( 'Timezone', 'cityo' ) ?>">
									...
									<div class="group-field-item-content ">
				    					...
				    					<div class="row flex-middle-lg">
								    				...
								    				<div class="enter-hours-item-inner">
								    					<div class="row">
										    				<div class="col-lg-6 col-xs-6">
											    				...
											    				<select class="select-job-hour-from" name="job_hours[day][<?php echo esc_attr($day); ?>][from][]" placeholder="<?php esc_attr_e( 'From', 'cityo' ) ?>">
																	<option value=""><?php esc_html_e( 'From', 'cityo' ) ?></option>
										    				...
										    				<div class="col-lg-6 col-xs-6">
										    					<div class="wrapper-select">
											    				<select class="select-job-hour-to" name="job_hours[day][<?php echo esc_attr($day); ?>][to][]" placeholder="<?php esc_attr_e( 'To', 'cityo' ) ?>">
																	<option value=""><?php esc_html_e( 'To', 'cityo' ) ?></option>
																	<?php foreach (range(0, 86399, 1800) as $time) {
																		$value = gmdate( 'H:i', $time);
																	?>
							    			...
							    			<div class="hours-last col-lg-5 col-xs-12">
										    	<label><input type="radio" name="job_hours[day][<?php echo esc_attr($day); ?>][type]" value="enter_hours" <?php echo trim(empty($type) || $type == 'enter_hours' ? 'checked="checked"' : ''); ?>> <?php esc_html_e('Enter Hours', 'cityo'); ?></label>
										    	<label><input type="radio" name="job_hours[day][<?php echo esc_attr($day); ?>][type]" value="open_all_day" <?php echo trim($type == 'open_all_day' ? 'checked="checked"' : ''); ?>> <?php esc_html_e('Open All Days', 'cityo'); ?></label>
										    	<label><input type="radio" name="job_hours[day][<?php echo esc_attr($day); ?>][type]" value="closed_all_day" <?php echo trim($type == 'closed_all_day' ? 'checked="checked"' : ''); ?>> <?php esc_html_e('Closed All Days', 'cityo'); ?></label>
										    </div>
									    </div>
					    			</div>
					    		</div>
				    	...
				    	<div class="row">
				    		...
				    		<div class="left-inner col-lg-11 col-xs-12">
				    			<div class="bottom-action-hour">
							    	<a class="add-new-hour btn-action button text-success" href="javascript:void(0);"><?php esc_html_e( 'Add New', 'cityo' ); ?></a>
									<a class="remove-hour btn-action button text-danger" href="javascript:void(0);"><?php esc_html_e( 'Remove', 'cityo' ); ?></a>
								</div>
				    		</div>
						</div>
				    </div>
