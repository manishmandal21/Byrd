...
$nb_fields = cityo_get_config('submit_listing_category_nb_fields', '1');
?>
...
<div class="categories-field <?php echo esc_attr( is_admin() ? 'is-admin' : ''); ?>">
	<?php
	    ...
	    $category1_text = cityo_get_config('submit_listing_category_1_field_label');
    ?>
	    <div class="field-category field-category1">
	    	<label><?php echo trim($category1_text); ?></label>
		    <select class="select-field-category select-field-category1" data-next="2" autocomplete="off" name="<?php echo isset( $field['name'] ) ? $field['name'] : $key; ?>[]" data-placeholder="<?php echo sprintf(esc_html__('Please select %s', 'cityo'), $category1_text); ?>">
		    	<option value=""><?php echo sprintf(esc_html__('Please select %s', 'cityo'), $category1_text); ?></option>
			    <?php
    ...
    <?php if ( $nb_fields == '2' ) {
    	$category2_text = cityo_get_config('submit_listing_category_2_field_label');
	?>
	    <div class="field-category field-category2">
	    	<label><?php echo trim($category2_text); ?></label>
	    	<select class="select-field-category select-field-category2" data-next="3" autocomplete="off" name="<?php echo isset( $field['name'] ) ? $field['name'] : $key; ?>[]" data-placeholder="<?php echo sprintf(esc_attr__('Please select %s', 'cityo'), $category2_text); ?>">
	    		<option value=""><?php echo sprintf(esc_html__('Please select %s', 'cityo'), $category2_text); ?></option>
	    		<?php
