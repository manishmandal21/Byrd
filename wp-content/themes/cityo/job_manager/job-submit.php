...
?>
<div class="clearfix">
	<h3 class="user-name"><?php echo esc_html__('Add Listing','cityo'); ?></h3>
	<form action="<?php echo esc_url( $action ); ?>" method="post" id="submit-job-form" class="job-manager-form" enctype="multipart/form-data">
		...
		<?php if ( job_manager_user_can_post_job() || job_manager_user_can_edit_job( $job_id ) ) : ?>
			...
			uasort( $all_fields, 'cityo_sort_array_by_priority' );
			...
			?>
					...
					<fieldset class="fieldset-<?php echo esc_attr( $key.' '.$field['type'] ); ?> <?php echo esc_attr($field['type'] == 'hidden' ? 'hidden' : ''); ?>">
						<?php if ( !empty($field['type']) && $field['type'] !== 'heading' && $field['type'] !== 'cityo-location' ) { ?>
			...
			<p class="clearfix">
				...
				<input type="submit" name="submit_job" class="button btn-submit btn btn-theme" value="<?php echo esc_html__('Save & Preview','cityo'); ?>" />
			</p>
