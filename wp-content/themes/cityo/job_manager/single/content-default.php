...
<div class="apus-single-listing">
	...
	<?php
	$cover_image = cityo_get_post_image_src($post->ID);
	if ( $cover_image ) { ?>
        <meta property="og:image" content="<?php echo esc_url($cover_image); ?>"/>
    <?php } ?>
	
	<?php if ( get_option( 'job_manager_hide_expired_content', 1 ) && 'expired' === $post->post_status ) : ?>
		<div class="job-manager-info"><?php esc_html_e( 'This listing has expired.', 'cityo' ); ?></div>
	<?php else : ?>
		<div class="row">
			...
			<div class="col-md-4">
				<div class="sidebar sidebar-detail-job sidebar-right">
					<div class="close-sidebar-btn hidden-lg hidden-md"> <i class="ti-close"></i> <span><?php esc_html_e('Close', 'cityo'); ?></span></div>
					...
					<?php
					$contents = cityo_get_sidebar_content_sort();
					if ( !empty($contents) ) {
						foreach ($contents as $key => $title) {
							echo trim(cityo_listing_display_part($key));
						}
					}
					?>
