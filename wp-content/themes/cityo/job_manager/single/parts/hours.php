...
$days = cityo_get_day_hours($hours);
...
?>
	<?php foreach ($days as $day => $times) {
			...
			$day_time_heading = '';
			if ( $times == 'open' ) {
				$day_time = '<span class="open-text">'.esc_html__('Open All Day', 'cityo').'</span>';
			} elseif ( $times == 'closed' ) {
				$day_time = '<span class="close-text">'.esc_html__('Closed All Day', 'cityo').'</span>';
			...
			<div id="listing-hours" class="listing-hours widget">
				<h2 class="widget-title">
					<div class="flex-bottom">
						<span><?php esc_html_e('Today', 'cityo'); ?></span>
						<?php cityo_display_time_status($post); ?>
				...
				<div class="listing-hours-inner1 flex-top-lg box-inner clearfix">
					...
					<?php do_action('cityo-single-listing-hours', $post); ?>
				</div>
			</div>
		<?php } ?>
<?php endif; ?>