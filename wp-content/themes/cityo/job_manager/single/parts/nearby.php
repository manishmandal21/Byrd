	...
	if ( !empty($term)) :
		...
		<div class="widget nearby">
			<h2 class="widget-title">
				<span><?php esc_html_e('Nearby listings', 'cityo'); ?></span>
			</h2>
			<div class="box-inner">
				<span><?php echo sprintf(esc_html__('Find more %s near', 'cityo'), $term->name); ?></span>
				<a href="<?php echo esc_url($link); ?>"><strong><?php the_title(); ?></strong></a>

				<?php do_action('cityo-single-listing-nearby', $post); ?>
			</div>
		</div>
	<?php
	endif;
